    ...
    public function process(File $phpcsFile, $stackPtr)
    {
        ...
        if ($tokens[$stackPtr]['code'] === T_DOC_COMMENT_OPEN_TAG) {
            ...
            if ($tokens[$stackPtr]['content'] === '/**' && $phpcsFile->tokenizerType !== 'JS') {
                ...
                if ($varTag === false) {
                    $error = 'CUSTOM - Inline doc block comments are not allowed; use "/* Comment */" or "// Comment" instead';
        ...
        if ($tokens[$stackPtr]['content'][0] === '#') {
            $error = 'CUSTOM - Perl-style comments are not allowed; use "// Comment" instead';
        ...
        foreach ($commentTokens as $lastCommentToken) {
            ...
            $fix = false;
            if ($tabFound === true) {
                $error = 'CUSTOM - Tab found before comment text; expected "// %s" but found "%s"';
                $data  = [
                ...
                $error = 'CUSTOM - No space found before comment text; expected "// %s" but found "%s"';
                $data  = [
                    substr($comment, 2),
                    $comment,
                ];
                    ...
                    if ($spaceCount > $prevSpaceCount && $prevSpaceCount > 0) {
                        ...
                        if (in_array($words[1], $indentationStarters) === true) {
                            if ($spaceCount !== ($prevSpaceCount + 2)) {
                                $error = 'CUSTOM - Comment indentation error after %s element, expected %s spaces';
                            ...
                            if ($spaceCount !== $expectedSpaceCount) {
                                $error = 'CUSTOM - Comment indentation error, expected %s spaces';
                            ...
                            $error = 'CUSTOM - Comment indentation error, expected only %s spaces';
                            $phpcsFile->addError($error, $lastCommentToken, 'SpacingBefore', [$prevSpaceCount]);
                        }//end if
                    }//end if
                } else {
                    $error = 'CUSTOM - %s spaces found before inline comment; expected "// %s" but found "%s"';
                    $data  = [
                        $spaceCount,
        ...
        if ($commentText === '') {
            $error = 'CUSTOM - Blank comments are not allowed';
        ...
        if (preg_match('/^\p{Ll}/u', $commentText) === 1) {
            ...
            if (isset($matches[0]) === true && $matches[0] === $words[0]) {
                $error = 'CUSTOM - Inline comments must start with a capital letter';
        ...
        if (preg_match('/^\p{L}/u', $commentText) === 1) {
            ...
            if (in_array($commentCloser, $acceptedClosers, true) === false
                ...
                $error = 'CUSTOM - Inline comments must end in %s';
                $ender = '';
        ...
        if ($tokens[$previousContent]['line'] < $tokens[$stackPtr]['line']) {
            ...
            $error = 'CUSTOM - There must be no blank line following an inline comment';
